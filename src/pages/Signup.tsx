...
type SignupProps = {
  ...
  /**
   * When set with `embedLandlordInvite`, hide name/email/password until the user
...
export default function Signup({
  ...
  const [accountKind, setAccountKind] = useState<SignupAccountKind | null>(() =>
    embedLandlordInvite || roleFromUrl === 'landlord'
      ? 'landlord'
  ...
  const [emailFieldsOpen, setEmailFieldsOpen] = useState(() => !(embedLandlordInvite && collapsedEmail))

  useScrollToTopOnChange(step, { anchorRef: formTopRef })

  useEffect(() => {
    if (!embedLandlordInvite) return
  ...
  }, [searchParams, embedLandlordInvite])
  ...
  if (checkEmail) {
    const redirectQCheck = searchParams.get('redirect')
    const loginBackHref =
      redirectQCheck && isSafeInternalPath(redirectQCheck)
        ? `/login?redirect=${encodeURIComponent(redirectQCheck)}`
        : '/login'
    return (
      <div className={embedLandlordInvite ? 'text-center' : 'max-w-md mx-auto px-6 py-12 text-center'}>
        {!embedLandlordInvite ? (
          <Seo
      ...
      <div ref={formTopRef} className="flex h-full min-h-0 flex-col scroll-mt-below-header">
        ...
        {collapsedEmail && !emailFieldsOpen ? (
          ...
          <>
            ...
            {showEmailFields ? (
              <form onSubmit={handleEmailSignup} className="mt-3 space-y-3">
                <div>
                  <label
