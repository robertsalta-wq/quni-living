...
export default function DeskPen({
  ...
  const base =
    'desk-pen inline-flex items-center justify-center gap-[var(--space-1)] font-bold transition-[transform,box-shadow,background-color,color] duration-[var(--dur-fast)] ease-[var(--ease-standard)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2'

  const styles: Record<NonNullable<DeskPenProps['variant']>, string> = {
    coral: [
      'w-full rounded-[var(--radius-md)] px-[var(--space-5)] py-[var(--space-2)] text-quni-body-sm',
      'bg-[var(--quni-coral)] text-white',
      'shadow-[0_3px_0_var(--quni-coral-active),var(--shadow-2)]',
      'hover:bg-[var(--quni-coral-hover)] hover:translate-y-[3px]',
      'hover:shadow-[0_0_0_var(--quni-coral-active),var(--shadow-1)]',
    ...
    ink: [
      'w-fit self-start rounded-[var(--radius-sm)] pt-[var(--space-2)] text-quni-caption',
      'text-[var(--quni-ink-2)] no-underline',
      'hover:text-[var(--quni-coral-active)]',
      'focus-visible:outline-[var(--quni-coral)]',
      '[&_.desk-pen-arw]:transition-transform [&_.desk-pen-arw]:duration-[var(--dur-base)]',
