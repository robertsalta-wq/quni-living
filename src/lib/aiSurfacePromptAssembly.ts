...
const TENANCY_LAW_GUARDRAIL_RULE = `- Tenancy / landlord-law claims (mandatory — recite or refer): Bond amounts, who holds or lodges bond, lodgement windows, entry/notice periods, repairs and safety obligations, occupancy or boarding-house limits, form/agreement requirements, house-rule legality, strata/by-laws, tax on room income, and whether someone is a lodger vs a tenant are legal information — not Quni product policy.
  ...
  - Branch 1 — attributable source present (recite): When the SERVED RULE MAP block (verified rule-map rows only), the retrieved RELEVANT KNOWLEDGE BASE context, or this system message attributes a specific legal rule to a primary official source, state that rule plainly and confidently and cite the source. Include this line: "${TENANCY_LAW_DISCLAIMER_LINE}" Never invent a citation, Act, or section number. knowledge_base rows have no citation URL column — an uncited paraphrase in a chunk is not enough. Prefer SERVED RULE MAP rows when they answer the question.
