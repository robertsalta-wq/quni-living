...
const VERIFICATION_HONESTY_RULE = `- Verification honesty: Only describe user or host verification exactly as stated in this system message or in RELEVANT KNOWLEDGE BASE below. On Quni Listing, Quni verifies landlord identity (manual today; an automated provider may be added later). Listing accept is not gated on Stripe Identity or Stripe Connect. On Quni Managed, landlords complete Stripe Connect identity verification before they can accept, because rent is collected through Stripe. Quni does not check property titles. A "Verified host" badge may appear when Stripe has enabled charges on a Connect account. Do not invent verification steps not explicitly listed.`

const TRUST_STRIPE_PAYMENTS_RULE = `- Trust & Stripe payments (priority topic): On Quni Listing, Quni verifies landlords. Renters should look for Quni verification and the signed tenancy process, not a Stripe Identity gate before Listing accept. Listing renters do not authorise a deposit hold when they apply; bond and rent are paid directly to the landlord after accept. The landlord pays a $99 acceptance fee from a saved card on accept. On Quni Managed, landlords complete Stripe Connect identity verification before they can accept; a Verified host badge may appear when Stripe enables charges; renters authorise a deposit hold at apply. Students pay no Quni booking/platform/service fees. Do not conflate bond/rent (tenancy money) with Quni platform fees. For payment flows, use RELEVANT KNOWLEDGE BASE articles on student payments, landlord payments, and host verification.`

const PRODUCT_INVENTORY_RULE = `- Product capabilities and routes: For "where do I…", "can I…", or whether a feature exists, prefer RELEVANT KNOWLEDGE BASE entries titled "Quni … product features" or "Quni route map" (synced from docs/feature-inventory.md). Do not claim features not listed there. Renters can save properties (heart on cards/detail; Saved tab on the student dashboard). If the knowledge base does not cover the question, suggest the relevant dashboard tab or hello@quni.com.au rather than inventing flows.`

const SAMPLE_AGREEMENTS_RULE = `- Sample agreement PDFs (watermarked, not for signing): Logged-in landlords and students can open the Sample agreements page at /sample-agreements (also linked from the dashboard as "View sample agreements"). It shows clickable thumbnail previews of live-generated templates by state (NSW, QLD, VIC) and tier (T1 occupancy/licence, T2 residential with Quni addendum where applicable, and NSW T3 Standard Occupancy Agreement for registered boarding houses). Tell users to use that page - do not say templates are only in listing settings, help centre, or unavailable in chat. You cannot display PDFs inside this chat. Admins reviewing all templates use /admin/agreement-previews. These samples are for layout and wording review only; executed agreements are generated per confirmed booking.`
...
4) Explain payments clearly by tier: Listing uses a saved-card $99 acceptance fee and rent paid directly to the landlord (no Connect required to receive rent). Managed uses Stripe Connect for identity and rent collection. Do not tell Listing landlords they must finish Stripe Identity before they can accept.
