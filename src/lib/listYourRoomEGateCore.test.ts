...
describe('listYourRoomEGateCore', () => {
  ...
  it('defaults on for Preview and off for Production', () => {
    expect(resolveListYourRoomEEnabled({ vercelEnv: 'preview' })).toBe(true)
    expect(resolveListYourRoomEEnabled({ vercelEnv: 'production' })).toBe(false)
  })

  it('honours explicit overrides', () => {
    expect(resolveListYourRoomEEnabled({ override: 'true', vercelEnv: 'production' })).toBe(true)
    expect(resolveListYourRoomEEnabled({ override: 'false', vercelEnv: 'preview' })).toBe(false)
  })
})
