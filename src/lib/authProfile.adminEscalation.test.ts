...
import { clearProfileHydrateInflight, fetchRoleAndProfile } from './authProfile'
...
describe('admin privilege escalation via user_metadata', () => {
  beforeEach(() => {
  ...
  it('does not resolve role=admin from spoofed user_metadata.role when RPC denies', async () => {
    const result = await fetchRoleAndProfile(spoofAdminUser())
  ...
  it('still resolves role=admin when platform_staff RPC affirms (null metadata)', async () => {
    vi.mocked(fetchIsPlatformAdmin).mockResolvedValue(true)

    const result = await fetchRoleAndProfile(
      spoofAdminUser({ user_metadata: {} }),
    )
