...
export default async function handler(req: any, res: any) {
  console.log('[generate-qld-residential-tenancy] incoming request', { method: req.method })
  ...
  console.log('[generate-qld-residential-tenancy] auth check', { match })
  ...
  if (propertyState !== 'QLD') {
    return res.status(400).json({ error: 'Property must be in Queensland (QLD) for Form 18a generation' })
  }
  ...
  try {
    ...
    console.error('[generate-qld-residential-tenancy] platform_config bank fetch', e)
  ...
  try {
    ...
    console.error('[generate-qld-residential-tenancy] platform_config communication emails', e)
  ...
  if (rtaUpErr) {
    ...
    return res.status(500).json({ error: 'Could not upload QLD Form 18a PDF' })
  }
  ...
  if (refreshDraftPdfsOnly) {
    return res.status(200).json({
      ...
      message:
        'Draft tenancy PDFs re-uploaded to Storage; DocuSeal submission unchanged. Downloaded signed copies stay as executed until a new signing round.',
    })
  }
