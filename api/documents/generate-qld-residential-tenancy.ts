...
/**
 * HTTP entry for QLD Form 18a generation (manual/admin retry).
 * Listing accept uses direct lib call via triggerListingDocumentGeneration.
 */
import { createClient } from '@supabase/supabase-js'
import type { VercelRequest, VercelResponse } from '@vercel/node'
import type { Database } from '../../src/lib/database.types.js'
import type { ListingDocGenResult } from '../lib/booking/listingAgreementTypes.js'
import { runQldForm18aListingTenancy } from '../lib/documents/listingTenancyGeneration/qldForm18a.js'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const result = await runQldForm18aListingTenancy(admin, bookingId, { deferSigning })
