...
/**
 * Generate VIC Form 1 (Residential rental agreement) + Quni Platform Addendum (VIC), upload to Storage,
...
/** Bundled from `src/lib/documents/vic/form1Generator.tsx` via `npm run build:api-documents`. */
import { VicResidentialRentalAgreementForm1 } from './VicForm1Agreement.js'
import { QuniPlatformAddendumVic } from './QuniPlatformAddendumVic.js'
...
export default async function handler(req: any, res: any) {
  console.log('[generate-vic-residential-rental] incoming request', { method: req.method })
  ...
  console.log('[generate-vic-residential-rental] auth check', { match })
  ...
  if (propertyState !== 'VIC') {
    return res.status(400).json({ error: 'Property must be in Victoria (VIC) for Form 1 generation' })
  }
  ...
  try {
    ...
    console.error('[generate-vic-residential-rental] platform_config bank fetch', e)
  ...
  try {
    ...
    console.error('[generate-vic-residential-rental] platform_config communication emails', e)
  ...
  try {
    ...
    console.error('[generate-vic-residential-rental] platform_config business identity', e)
  ...
  if (rtaUpErr) {
    ...
    return res.status(500).json({ error: 'Could not upload VIC Form 1 PDF' })
  }
