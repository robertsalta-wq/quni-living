...
import { confirmReinstatement } from '../../lib/booking/reinstatement/confirmReinstatement.js'
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    return corsJson(
      ...
      {
        ok: true,
        alreadyReinstated: Boolean(result.alreadyReinstated),
        request: result.request,
        bookingStatusAfter: result.bookingStatusAfter,
        signing_needs_resend: result.signing_needs_resend,
        signing_resend_failed: result.signing_resend_failed,
        fee_action: result.fee_action,
        listing_fee_refunded: result.listing_fee_refunded,
      },
      200,
      origin,
    )
  } catch (e) {
    console.error('[api/booking/reinstatement/confirm]', e)
