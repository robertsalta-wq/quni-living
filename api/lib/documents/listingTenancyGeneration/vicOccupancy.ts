...
type LoadedVicOccupancyContext = {
  booking: BookingRow
...
async function loadVicOccupancyContext(
  ...
  | { ok: true; ctx: LoadedVicOccupancyContext }
...
> {
  ...
  if (stateRaw !== 'VIC') {
    return { ok: false, status: 400, error: 'Property must be in Victoria for VIC licence to occupy PDF' }
  }
...
function buildVicOccupancyPdfProps(ctx: LoadedVicOccupancyContext, documentId: string): OccupancyAgreementProps {
...
async function buildVicOccupancyPdfBuffer(ctx: LoadedVicOccupancyContext, documentId: string): Promise<Buffer> {
  const pdfProps = buildVicOccupancyPdfProps(ctx, documentId)
...
export async function preflightVicOccupancyListingTenancy(
  admin: SupabaseClient<Database>,
  bookingId: string,
): Promise<ListingPreflightResult> {
  const loaded = await loadVicOccupancyContext(admin, bookingId, { requireConfirmable: false })
  if (isListingContextLoadFail(loaded)) {
    return listingContextLoadFailure(loaded)
  }
  try {
    await buildVicOccupancyPdfBuffer(loaded.ctx, PREFLIGHT_DOCUMENT_ID)
    return { ok: true, generator: 'vic-occupancy' }
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    console.error('[vic-occupancy] preflight pdf build', e)
    return { ok: false, status: 500, error: 'Could not build occupancy agreement PDF', detail: msg }
  }
}

export async function runVicOccupancyListingTenancy(
  admin: SupabaseClient<Database>,
  bookingId: string,
  opts: { deferSigning: boolean },
): Promise<ListingDocGenResult> {
  const loaded = await loadVicOccupancyContext(admin, bookingId)
  if (isListingContextLoadFail(loaded)) {
    return listingContextLoadFailure(loaded)
  }
  ...
  try {
    pdfBuffer = await buildVicOccupancyPdfBuffer(loaded.ctx, documentId)
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    return { ok: false, status: 500, error: 'Could not build occupancy agreement PDF', detail: msg }
  }
  ...
  if (upErr) {
    console.error('storage upload vic occupancy draft', upErr)
