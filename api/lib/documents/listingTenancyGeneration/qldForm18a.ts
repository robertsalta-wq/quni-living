...
import { fillOfficialQldForm18aPdf } from '../officialQldForm18aFill.js'
import { QuniPlatformAddendumQld } from '../../../documents/QuniPlatformAddendumQld.js'
...
> {
  ...
  if (propertyState !== 'QLD') {
    return { ok: false, status: 400, error: 'Property must be in Queensland (QLD) for Form 18a generation' }
  }
  ...
  try {
    ...
    console.error('[qld-form18a] platform_config bank fetch', e)
    await captureSentryMessageEdge('QLD Form 18a: failed to load platform_config for bank details', {
  ...
  if (!bankDetails.bsb || !bankDetails.accountNumber) {
    await captureSentryMessageEdge(
      'QLD Form 18a: bank BSB or account number missing in platform_config',
      { booking_id: bookingId },
    )
    return {
      ok: false,
  ...
  try {
    ...
    console.error('[qld-form18a] platform_config communication emails', e)
    await captureSentryMessageEdge('QLD Form 18a: failed to load platform_config for addendum emails', {
      booking_id: bookingId,
      error: e instanceof Error ? e.message : String(e),
    })
  }
  ...
  try {
    ...
    console.error('[qld-form18a] platform_config business identity', e)
...
function buildQldPdfProps(ctx: LoadedQldContext, documentId: string) {
  ...
  return { form18aProps, addendumProps }
}

async function buildQldForm18aPdfBuffers(ctx: LoadedQldContext, documentId: string): Promise<{
  form18aBuffer: Buffer
  addendumBuffer: Buffer
}> {
  const { form18aProps, addendumProps } = buildQldPdfProps(ctx, documentId)
  ...
  if (!form18aBuffer.length || !addendumBuffer.length) {
    throw new Error('Generated PDF buffers are empty')
  }
  return { form18aBuffer, addendumBuffer }
}

export async function preflightQldForm18aListingTenancy(
  admin: SupabaseClient<Database>,
  bookingId: string,
  ...
  try {
    ...
    console.error('[qld-form18a] preflight pdf build', e)
    return { ok: false, status: 500, error: 'Could not build tenancy agreement PDF', detail: msg }
  }
}

export async function runQldForm18aListingTenancy(
  admin: SupabaseClient<Database>,
  bookingId: string,
  ...
  if (existingRta?.id) {
    ...
    const { data: insD, error: dErr } = await admin
      ...
      .insert({
        ...
        metadata: { signing_package: 'residential_tenancy_qld' } as Json,
      })
      .select('id')
      .single()
  ...
  let form18aBuffer: Buffer
  let addendumBuffer: Buffer
  try {
    const built = await buildQldForm18aPdfBuffers(loaded.ctx, documentId)
    form18aBuffer = built.form18aBuffer
  ...
  const { error: rtaUpErr } = await admin.storage
    ...
    .upload(rtaStoragePath, form18aBuffer, { contentType: 'application/pdf', upsert: true })

  if (rtaUpErr) {
    console.error('storage upload QLD Form 18a draft', rtaUpErr)
    return { ok: false, status: 500, error: 'Could not upload QLD Form 18a PDF' }
  }
  ...
  if (addUpErr) {
    console.error('storage upload QLD addendum draft', addUpErr)
  ...
  const { error: pathErr } = await admin
    ...
    .update({
      ...
      status: 'draft',
      metadata: {
        signing_package: 'residential_tenancy_qld',
