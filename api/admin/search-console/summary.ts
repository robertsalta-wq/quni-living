...
import { fetchSearchConsoleSummaryFromGoogle } from '../../lib/googleSearchConsole.js'
import {
  json,
...
import type { SearchConsoleSummaryPayload } from '../../lib/googleSearchConsole.js'

export const config = { runtime: 'nodejs', maxDuration: 60 }

const CACHE_KEY = 'search-console/summary'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (!bypassCache) {
      const row = readSearchConsoleCacheMemory<SearchConsoleSummaryPayload>(CACHE_KEY)
      if (row?.fresh) {
    ...
    const data = await fetchSearchConsoleSummaryFromGoogle()
    ...
    const message = e instanceof Error ? e.message : 'Unable to fetch Search Console summary'
    console.error('[api/admin/search-console/summary]', e)
